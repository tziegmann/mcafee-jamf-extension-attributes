...
## mcafee-ens-webcontrol-hotfix-version-ea.sh
## version 1.0
## 
...
FMHotfix=`cat /usr/local/McAfee/fmp/config/WebProtection/FMConfig.xml | egrep "<Hotfixes>.*</Hotfixes>" |sed -e "s/<Hotfixes>\(.*\)<\/Hotfixes>/\1/"|tr -d " "|tr -d "\t"|tr -d "\n"|tr -d "\r"`
